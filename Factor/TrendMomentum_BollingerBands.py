...
# Factor Name: TrendMomentum_BollingerBands
...
FACTOR_NAME = 'TrendMomentum_BollingerBands'
...
def _calc_signal(df, n_value):
    close = df['adj_close']
    ma = close.rolling(window=n_value).mean()
    std = close.rolling(window=n_value).std()
    z = (close - ma) / (2 * std + 1e-8)
    return z.fillna(0)
