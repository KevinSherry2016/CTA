...
# 参数列表 (默认候选)
    ...
    # ========== 回测指定设置 ==========
    # 单一新增过滤器/增强器：OI(持仓量)变化确认软过滤与同向增强
    USE_OI_FILTER = True
    OI_SOFT_WEIGHT = 0.5
    OI_ENHANCE_WEIGHT = 1.5
    ...
    for sector, ts_codes in sector_map.items():
        ...
        for N in test_n_list:
            ...
            for ts_code in valid_symbols:
                ...
                # OI过滤与增强：过去N天持仓量(OI)下降软过滤，上升则增强
                weight = pd.Series(1.0, index=raw_sig.index)
                if USE_OI_FILTER:
                    oi_roc = df['oi'].pct_change(N).fillna(0)
                    weight.loc[oi_roc <= 0] = OI_SOFT_WEIGHT
                    weight.loc[oi_roc > 0] = OI_ENHANCE_WEIGHT
                # --------------------
    ...
    report_path = os.path.join('./Result', "TrendMomentum_TimeSeriesMomentum_OIFilter_BacktestResult.csv")
    ...
    if best_positions_noz:
        ...
        df_pos_noz.to_csv(os.path.join('./Result', f"TrendMomentum_TimeSeriesMomentum_OIFilter_NoZscore_Position{filename_suffix}.csv"), encoding='utf-8-sig')
    if best_positions_z:
        df_pos_z = pd.DataFrame(best_positions_z, index=tradingDayList).fillna(0)
        df_pos_z.to_csv(os.path.join('./Result', f"TrendMomentum_TimeSeriesMomentum_OIFilter_Zscore_Position{filename_suffix}.csv"), encoding='utf-8-sig')
    if best_positions_sm:
        df_pos_sm = pd.DataFrame(best_positions_sm, index=tradingDayList).fillna(0)
        df_pos_sm.to_csv(os.path.join('./Result', f"TrendMomentum_TimeSeriesMomentum_OIFilter_StateMachine_Position{filename_suffix}.csv"), encoding='utf-8-sig')
