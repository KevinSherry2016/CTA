...
def _calc_signal(df, n_value):
    close = df['adj_close']
    ret = close.diff()
    gain = (ret.where(ret > 0, 0)).rolling(window=n_value).mean()
    loss = (-ret.where(ret < 0, 0)).rolling(window=n_value).mean()
    rs = gain / (loss + 1e-9)
    rsi = 100 - (100 / (1 + rs))
    return (rsi - 50) / 100
