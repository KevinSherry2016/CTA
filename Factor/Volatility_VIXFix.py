...
def _calc_signal(df, n_value):
    close = df['adj_close']
    low = df['adj_low']
    highest_close = close.rolling(window=n_value, min_periods=1).max()
    raw = ((highest_close - low) / (highest_close + 1e-8) * 100).fillna(0)
    signal = raw / 50 - 1
    return signal
