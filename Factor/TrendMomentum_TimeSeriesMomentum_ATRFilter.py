...
# 参数列表 (默认候选)
    ...
    # ========== 回测指定设置 ==========
    # 单一新增过滤器/增强器：ATR波动率扩张确认软过滤与同向增强
    USE_ATR_FILTER = True
    ATR_SHORT = 5
    ATR_LONG = 20
    ATR_SOFT_WEIGHT = 0.5
        ...
        for N in test_n_list:
            ...
            for ts_code in valid_symbols:
                ...
                raw_sig = close.pct_change(N).fillna(0)
                                ...
                                # --------------------
                # ATR波动率过滤与增强：波动率收缩削弱信号，波动率扩张增强信号
    ...
    report_path = os.path.join('./Result', "TrendMomentum_TimeSeriesMomentum_ATRFilter_BacktestResult.csv")
    ...
    if best_positions_noz:
        ...
        df_pos_noz.to_csv(os.path.join('./Result', f"TrendMomentum_TimeSeriesMomentum_ATRFilter_NoZscore_Position{filename_suffix}.csv"), encoding='utf-8-sig')
    if best_positions_z:
        df_pos_z = pd.DataFrame(best_positions_z, index=tradingDayList).fillna(0)
        df_pos_z.to_csv(os.path.join('./Result', f"TrendMomentum_TimeSeriesMomentum_ATRFilter_Zscore_Position{filename_suffix}.csv"), encoding='utf-8-sig')
    if best_positions_sm:
        df_pos_sm = pd.DataFrame(best_positions_sm, index=tradingDayList).fillna(0)
        df_pos_sm.to_csv(os.path.join('./Result', f"TrendMomentum_TimeSeriesMomentum_ATRFilter_StateMachine_Position{filename_suffix}.csv"), encoding='utf-8-sig')
