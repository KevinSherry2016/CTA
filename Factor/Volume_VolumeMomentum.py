...
def main():
    ...
    # 3. 提取日度行情数据
    data = {}
    print(f"正在加载 Volume_VolumeMomentum 数据...")
    ...
    print(f"开始计算 Volume_VolumeMomentum 因子信号...")
    for ts_code, df in data.items():
        ...
        # 2. 计算成交量动量并处理换月失真：只要过去N天内发生过换月，直接剔除并用 ffill 继承之前信号
        invalid_window = is_rollover.rolling(window=N, min_periods=1).max() > 0
        
        signal = volume / volume.rolling(window=N).mean() - 1
    ...
    output_name = f"Volume_VolumeMomentum.csv"
    output_path = os.path.join('./Result', output_name)
    signals.to_csv(output_path, encoding='utf-8-sig')
    print(f"因子 Volume_VolumeMomentum 输出完成: {output_path}")
