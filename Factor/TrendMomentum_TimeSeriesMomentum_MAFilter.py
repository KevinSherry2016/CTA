...
# 参数列表 (默认候选)
    ...
    # ========== 回测指定设置 ==========
    # 单一新增过滤器/增强器：大势(MA大周期)一致性确认软过滤与同向增强
    USE_MA_FILTER = True
    MA_WINDOW = 120
    MA_SOFT_WEIGHT = 0.5
    MA_ENHANCE_WEIGHT = 1.5
    ...
    for sector, ts_codes in sector_map.items():
        ...
        for N in test_n_list:
            ...
            for ts_code in valid_symbols:
                ...
                if USE_MA_FILTER:
                    ma_trend = close.rolling(MA_WINDOW).mean()
                    # 逆势过滤
                    weight.loc[(raw_sig > 0) & (close < ma_trend)] = MA_SOFT_WEIGHT
                    weight.loc[(raw_sig < 0) & (close > ma_trend)] = MA_SOFT_WEIGHT
                    # 顺势增强
                    weight.loc[(raw_sig > 0) & (close > ma_trend)] = MA_ENHANCE_WEIGHT
                    weight.loc[(raw_sig < 0) & (close < ma_trend)] = MA_ENHANCE_WEIGHT
                # --------------------
    ...
    report_path = os.path.join('./Result', "TrendMomentum_TimeSeriesMomentum_MAFilter_BacktestResult.csv")
    ...
    if best_positions_noz:
        ...
        df_pos_noz.to_csv(os.path.join('./Result', f"TrendMomentum_TimeSeriesMomentum_MAFilter_NoZscore_Position{filename_suffix}.csv"), encoding='utf-8-sig')
    if best_positions_z:
        df_pos_z = pd.DataFrame(best_positions_z, index=tradingDayList).fillna(0)
        df_pos_z.to_csv(os.path.join('./Result', f"TrendMomentum_TimeSeriesMomentum_MAFilter_Zscore_Position{filename_suffix}.csv"), encoding='utf-8-sig')
    if best_positions_sm:
        df_pos_sm = pd.DataFrame(best_positions_sm, index=tradingDayList).fillna(0)
        df_pos_sm.to_csv(os.path.join('./Result', f"TrendMomentum_TimeSeriesMomentum_MAFilter_StateMachine_Position{filename_suffix}.csv"), encoding='utf-8-sig')
