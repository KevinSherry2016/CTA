# ==============================================================================
# 因子类别：TrendMomentum
# 因子名称：TrendMomentum_MovingAverageBias
# 代表意义：价格偏离N日均线的程度
# ==============================================================================
...
def main():
    ...
    # 3. 提取日度行情数据
    data = {}
    print(f"正在加载 TrendMomentum_MovingAverageBias 数据...")
    ...
    print(f"开始计算 TrendMomentum_MovingAverageBias 因子信号...")
    for ts_code, df in data.items():
        ...
        # --- 因子计算逻辑 ---
        signal = close / close.rolling(window=N).mean() - 1
        # --------------------
    ...
    output_name = f"TrendMomentum_MovingAverageBias.csv"
    output_path = os.path.join('./Result', output_name)
    signals.to_csv(output_path, encoding='utf-8-sig')
    print(f"因子 TrendMomentum_MovingAverageBias 输出完成: {output_path}")
