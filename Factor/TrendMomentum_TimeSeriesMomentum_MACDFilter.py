# ==============================================================================
# 因子类别：TrendMomentum
# 因子名称：TrendMomentum_TimeSeriesMomentum_MACDFilter
# 代表意义：时间序列动量(N日收益率) + 单一MACD软过滤
# ==============================================================================
...
# 参数列表 (默认候选)
    ...
    for sector, ts_codes in sector_map.items():
        ...
        for N in test_n_list:
            ...
            for ts_code in valid_symbols:
                ...
                if USE_MACD_FILTER:
                    macd = calc_macd(close, MACD_FAST, MACD_SLOW)
                    weight = pd.Series(1.0, index=raw_sig.index)
                    # 软过滤：如果动量方向和MACD方向不一致，降低信号强度
                    weight.loc[(raw_sig > 0) & (macd <= 0)] = MACD_SOFT_WEIGHT
                    weight.loc[(raw_sig < 0) & (macd >= 0)] = MACD_SOFT_WEIGHT
    ...
    report_path = os.path.join('./Result', "TrendMomentum_TimeSeriesMomentum_MACDFilter_BacktestResult.csv")
    ...
    if best_positions_noz:
        ...
        df_pos_noz.to_csv(os.path.join('./Result', f"TrendMomentum_TimeSeriesMomentum_MACDFilter_NoZscore_Position{filename_suffix}.csv"), encoding='utf-8-sig')
    if best_positions_z:
        df_pos_z = pd.DataFrame(best_positions_z, index=tradingDayList).fillna(0)
        df_pos_z.to_csv(os.path.join('./Result', f"TrendMomentum_TimeSeriesMomentum_MACDFilter_Zscore_Position{filename_suffix}.csv"), encoding='utf-8-sig')
    if best_positions_sm:
        df_pos_sm = pd.DataFrame(best_positions_sm, index=tradingDayList).fillna(0)
        df_pos_sm.to_csv(os.path.join('./Result', f"TrendMomentum_TimeSeriesMomentum_MACDFilter_StateMachine_Position{filename_suffix}.csv"), encoding='utf-8-sig')
