...
# Factor Name: MeanReversion_VWAPDistance
...
FACTOR_NAME = 'MeanReversion_VWAPDistance'
...
def _calc_signal(df, n_value):
    ...
    vwap = (typical_price * volume).rolling(window=n_value, min_periods=1).sum() / (
        volume.rolling(window=n_value, min_periods=1).sum() + 1e-8
    )
    return (-(close / (vwap + 1e-8) - 1)).fillna(0)
