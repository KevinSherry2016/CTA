...
def main():
    ...
    print(f"开始计算 TrendMomentum_TimeSeriesMomentum 因子信号...")
    for ts_code, df in data.items():
        ...
        # --- 因子计算逻辑 ---
        # 时序动量：N日收益率
        signal = close.pct_change(N).fillna(0)
        # --------------------
    ...
    output_name = f"TrendMomentum_TimeSeriesMomentum.csv"
    output_path = os.path.join('./Result', output_name)
    signals.to_csv(output_path, encoding='utf-8-sig')
    print(f"因子 TrendMomentum_TimeSeriesMomentum 输出完成: {output_path}")
