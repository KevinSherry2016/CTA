...
# Factor Name: MeanReversion_VolumeShockReversal
...
FACTOR_NAME = 'MeanReversion_VolumeShockReversal'
...
def _calc_signal(df, n_value):
    ...
    vol_ma = volume.rolling(window=n_value).mean()
    vol_std = volume.rolling(window=n_value).std()
    vol_z = (volume - vol_ma) / (vol_std + 1e-8)

    # High volume with one-sided move is often followed by short-term pullback.
    raw = -(ret * vol_z)
    return raw.rolling(window=n_value).mean().fillna(0)
