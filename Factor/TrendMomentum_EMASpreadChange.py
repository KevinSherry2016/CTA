...
# Factor Name: TrendMomentum_EMASpreadChange
...
FACTOR_NAME = 'TrendMomentum_EMASpreadChange'
...
def _calc_signal(df, n_value):
    close = df['adj_close']
    fast_n = max(2, n_value // 2)
    slow_n = max(fast_n + 1, n_value)
    fast = close.ewm(span=fast_n, adjust=False).mean()
    slow = close.ewm(span=slow_n, adjust=False).mean()
    spread = fast / (slow + 1e-8) - 1
    return spread.diff().fillna(0)
