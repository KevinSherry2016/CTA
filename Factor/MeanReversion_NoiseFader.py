...
# Factor Name: MeanReversion_NoiseFader
...
FACTOR_NAME = 'MeanReversion_NoiseFader'
...
def _calc_signal(df, n_value):
    close = df['adj_close']
    direction = (close - close.shift(n_value)).abs()
    volatility = close.diff().abs().rolling(window=n_value, min_periods=1).sum()
    ker = direction / (volatility + 1e-8)
    trend_dir = np.sign(close - close.shift(n_value))
    return (-trend_dir * (1 - ker)).fillna(0)
