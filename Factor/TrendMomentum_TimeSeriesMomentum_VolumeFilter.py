...
# 参数列表 (默认候选)
    ...
    # ========== 回测指定设置 ==========
    # 单一新增过滤器/增强器：成交量活跃度确认软过滤与增强
    USE_VOL_FILTER = True
    VOL_SHORT = 5
    VOL_LONG = 20
    VOL_SOFT_WEIGHT = 0.5
    VOL_ENHANCE_WEIGHT = 1.5
    ...
    for sector, ts_codes in sector_map.items():
        ...
        for N in test_n_list:
            ...
            for ts_code in valid_symbols:
                ...
                # 成交量过滤与增强：清淡削弱趋势信号，活跃增强
                weight = pd.Series(1.0, index=raw_sig.index)
                if USE_VOL_FILTER:
                    vol_ma_short = df['vol'].rolling(VOL_SHORT).mean()
                    vol_ma_long = df['vol'].rolling(VOL_LONG).mean()
                    weight.loc[vol_ma_short < vol_ma_long] = VOL_SOFT_WEIGHT
                    weight.loc[vol_ma_short > vol_ma_long] = VOL_ENHANCE_WEIGHT
                # --------------------
    ...
    report_path = os.path.join('./Result', "TrendMomentum_TimeSeriesMomentum_VolumeFilter_BacktestResult.csv")
    ...
    if best_positions_noz:
        ...
        df_pos_noz.to_csv(os.path.join('./Result', f"TrendMomentum_TimeSeriesMomentum_VolumeFilter_NoZscore_Position{filename_suffix}.csv"), encoding='utf-8-sig')
    if best_positions_z:
        df_pos_z = pd.DataFrame(best_positions_z, index=tradingDayList).fillna(0)
        df_pos_z.to_csv(os.path.join('./Result', f"TrendMomentum_TimeSeriesMomentum_VolumeFilter_Zscore_Position{filename_suffix}.csv"), encoding='utf-8-sig')
    if best_positions_sm:
        df_pos_sm = pd.DataFrame(best_positions_sm, index=tradingDayList).fillna(0)
        df_pos_sm.to_csv(os.path.join('./Result', f"TrendMomentum_TimeSeriesMomentum_VolumeFilter_StateMachine_Position{filename_suffix}.csv"), encoding='utf-8-sig')
