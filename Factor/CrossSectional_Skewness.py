...
def main():
    ...
    # 3. 提取日度行情数据
    data = {}
    print(f"正在加载 CrossSectional_Skewness 数据...")
    ...
    print(f"开始计算 CrossSectional_Skewness 因子信号...")
    for ts_code, df in data.items():
        ...
        signal = ret.rolling(window=N).skew()
        # --------------------
    ...
    output_name = f"CrossSectional_Skewness.csv"
    output_path = os.path.join('./Result', output_name)
    signals.to_csv(output_path, encoding='utf-8-sig')
    print(f"因子 CrossSectional_Skewness 输出完成: {output_path}")
