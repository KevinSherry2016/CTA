...
def _calc_signal(df, n_value):
    ...
    gap = open_price / (prev_close + 1e-8) - 1
    intraday_ret = close / (open_price + 1e-8) - 1

    close_location = ((close - low) - (high - close)) / (high - low + 1e-8)
    gap_fill_component = -np.sign(gap) * intraday_ret

    signal = (gap_fill_component * (1 + close_location)).rolling(window=n_value, min_periods=1).mean()
