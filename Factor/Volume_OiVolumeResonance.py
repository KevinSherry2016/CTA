...
# Factor Name: Volume_OiVolumeResonance
...
FACTOR_NAME = 'Volume_OiVolumeResonance'
...
def _calc_signal(df, n_value):
    ...
    oi = df.get('oi', df.get('OpenInterest', pd.Series(dtype=float, index=df.index))).astype(float)

    ret_n = close.pct_change(periods=n_value)
    vol_chg = volume.pct_change().replace([np.inf, -np.inf], np.nan)
    oi_chg = oi.pct_change().replace([np.inf, -np.inf], np.nan)

    participation_sync = oi_chg.rolling(window=n_value, min_periods=max(3, n_value // 2)).corr(vol_chg)
    pressure = (oi_chg + vol_chg).rolling(window=max(2, n_value // 2), min_periods=1).mean()

    signal = ret_n * participation_sync * np.tanh(5 * pressure)
