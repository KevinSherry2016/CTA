# ==============================================================================
# 因子类别：TrendMomentum
# 因子名称：TrendMomentum_DonchianChannel
# 代表意义：唐奇安通道位置
# ==============================================================================
...
def main():
    ...
    # 3. 提取日度行情数据
    data = {}
    print(f"正在加载 TrendMomentum_DonchianChannel 数据...")
    ...
    print(f"开始计算 TrendMomentum_DonchianChannel 因子信号...")
    for ts_code, df in data.items():
        ...
        # --- 因子计算逻辑 ---
        roll_max = high.rolling(window=N).max()
        roll_min = low.rolling(window=N).min()
        signal = (close - roll_min) / (roll_max - roll_min + 1e-9) - 0.5
        # --------------------
    ...
    output_name = f"TrendMomentum_DonchianChannel.csv"
    output_path = os.path.join('./Result', output_name)
    signals.to_csv(output_path, encoding='utf-8-sig')
    print(f"因子 TrendMomentum_DonchianChannel 输出完成: {output_path}")
