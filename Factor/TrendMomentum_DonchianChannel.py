...
# Factor Name: TrendMomentum_DonchianChannel
...
FACTOR_NAME = 'TrendMomentum_DonchianChannel'
...
def _calc_signal(df, n_value):
    ...
    high = df['adj_high']
    low = df['adj_low']
    roll_max = high.rolling(window=n_value).max()
    roll_min = low.rolling(window=n_value).min()
    return (close - roll_min) / (roll_max - roll_min + 1e-9) - 0.5
