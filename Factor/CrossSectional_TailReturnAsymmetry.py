# ==============================================================================
# Factor Category: CrossSectional
# Factor Name: CrossSectional_TailReturnAsymmetry
...
FACTOR_NAME = 'CrossSectional_TailReturnAsymmetry'
...
def _calc_signal(df, n_value):
    ...
    min_periods = max(3, n_value // 2)
    q_low = ret.rolling(window=n_value, min_periods=min_periods).quantile(0.2)
    q_high = ret.rolling(window=n_value, min_periods=min_periods).quantile(0.8)

    down_tail = ret.where(ret <= q_low, 0.0)
    up_tail = ret.where(ret >= q_high, 0.0)

    down_mean = down_tail.rolling(window=n_value, min_periods=1).mean().abs()
    up_mean = up_tail.rolling(window=n_value, min_periods=1).mean()

    signal = up_mean / (down_mean + 1e-8) - 1
