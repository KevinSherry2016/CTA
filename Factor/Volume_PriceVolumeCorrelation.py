...
def main():
    ...
    for ts_code in info['ts_code']:
        ...
        if os.path.exists(filepath):
            ...
            for col in ['open', 'high', 'low', 'close', 'adj_close', 'adj_open', 'adj_high', 'adj_low', 'vol', 'oi', 'Volume', 'OpenInterest']:
    ...
    for sector, ts_codes in sector_map.items():
        ...
        for N in test_n_list:
            ...
            for ts_code in valid_symbols:
                ...
                raw_sig = daily_ret.rolling(window=N).corr(vol_chg).replace([np.inf, -np.inf], np.nan).fillna(0)
                # --------------------
    ...
    report_path = os.path.join('./Result', "Volume_PriceVolumeCorrelation_BacktestResult.csv")
    ...
    if best_positions_noz:
        ...
        df_pos_noz.to_csv(os.path.join('./Result', f"Volume_PriceVolumeCorrelation_NoZscore_Position{filename_suffix}.csv"), encoding='utf-8-sig')
    if best_positions_z:
        df_pos_z = pd.DataFrame(best_positions_z, index=tradingDayList).fillna(0)
        df_pos_z.to_csv(os.path.join('./Result', f"Volume_PriceVolumeCorrelation_Zscore_Position{filename_suffix}.csv"), encoding='utf-8-sig')
    if best_positions_sm:
        df_pos_sm = pd.DataFrame(best_positions_sm, index=tradingDayList).fillna(0)
        df_pos_sm.to_csv(os.path.join('./Result', f"Volume_PriceVolumeCorrelation_StateMachine_Position{filename_suffix}.csv"), encoding='utf-8-sig')
