...
# Factor Name: Volume_PriceVolumeCorrelation
...
FACTOR_NAME = 'Volume_PriceVolumeCorrelation'
...
def _calc_signal(df, n_value):
    ...
    return daily_ret.rolling(window=n_value).corr(vol_chg).replace([np.inf, -np.inf], np.nan).fillna(0)
