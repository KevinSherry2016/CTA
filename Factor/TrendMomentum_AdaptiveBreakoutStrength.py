...
# Factor Name: TrendMomentum_AdaptiveBreakoutStrength
...
FACTOR_NAME = 'TrendMomentum_AdaptiveBreakoutStrength'
...
def _calc_signal(df, n_value):
    ...
    lookback = max(2, n_value)
    min_periods = max(2, lookback // 2)

    prev_high = high.shift(1).rolling(window=lookback, min_periods=min_periods).max()
    prev_low = low.shift(1).rolling(window=lookback, min_periods=min_periods).min()
    breakout_pos = (close - prev_low) / (prev_high - prev_low + 1e-8) - 0.5

    fast_span = max(2, lookback // 2)
    ema_fast = close.ewm(span=fast_span, adjust=False).mean()
    ema_slow = close.ewm(span=lookback, adjust=False).mean()
    trend_strength = ema_fast / (ema_slow + 1e-8) - 1

    signal = breakout_pos * np.tanh(10 * trend_strength)
