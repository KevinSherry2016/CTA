...
def main():
    ...
    # 3. 提取日度行情数据
    data = {}
    print(f"正在加载 Volume_OpenInterestROC 数据...")
    ...
    print(f"开始计算 Volume_OpenInterestROC 因子信号...")
    ...
    output_name = f"Volume_OpenInterestROC.csv"
    output_path = os.path.join('./Result', output_name)
    signals.to_csv(output_path, encoding='utf-8-sig')
    print(f"因子 Volume_OpenInterestROC 输出完成: {output_path}")
