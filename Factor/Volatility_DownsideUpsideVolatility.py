# ==============================================================================
# 因子类别：Volatility
# 因子名称：Volatility_DownsideUpsideVolatility
# 代表意义：上下行波动率倾向
# ==============================================================================
...
def main():
    ...
    # 3. 提取日度行情数据
    data = {}
    print(f"正在加载 Volatility_DownsideUpsideVolatility 数据...")
    ...
    output_name = f"Volatility_DownsideUpsideVolatility.csv"
    output_path = os.path.join('./Result', output_name)
    signals.to_csv(output_path, encoding='utf-8-sig')
    print(f"因子 Volatility_DownsideUpsideVolatility 输出完成: {output_path}")
