# ==============================================================================
# Factor Category: Volatility
# Factor Name: Volatility_DownsideUpsideVolatility
...
FACTOR_NAME = 'Volatility_DownsideUpsideVolatility'
...
def _calc_signal(df, n_value):
    ...
    down_vol = (-ret.where(ret < 0, 0)).rolling(window=n_value).std()
    up_vol = (ret.where(ret > 0, 0)).rolling(window=n_value).std()
    rs = up_vol / (down_vol + 1e-9)
    vol_rsi = 100 - (100 / (1 + rs))
    return (vol_rsi - 50) / 100
