...
# Factor Name: MeanReversion_ExhaustionReversalComposite
...
FACTOR_NAME = 'MeanReversion_ExhaustionReversalComposite'
...
def _calc_signal(df, n_value):
    ...
    vol_mean = volume.rolling(window=n_value, min_periods=1).mean()
    ...
    exhaustion = ret * vol_z * (1 + intraday_range.clip(lower=0))
    reversal_pressure = exhaustion + 0.5 * wick_bias

    smooth_window = max(2, n_value // 2)
    signal = -reversal_pressure.rolling(window=smooth_window, min_periods=1).mean()
