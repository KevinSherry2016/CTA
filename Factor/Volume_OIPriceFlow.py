...
# Factor Name: Volume_OIPriceFlow
...
FACTOR_NAME = 'Volume_OIPriceFlow'
...
def _calc_signal(df, n_value):
    close = df['adj_close']
    oi = df.get('oi', pd.Series(dtype=float, index=df.index)).astype(float)
    ret = close.pct_change()
    oi_chg = oi.pct_change().replace([np.inf, -np.inf], np.nan)
    flow = np.sign(ret) * oi_chg
    return flow.rolling(window=n_value).mean().fillna(0)
