...
2. **参数寻优范围 (Parameter Expansion)：
   - **单参数因子**将原有的 `[10, 20, 30]` 扩展到 `[10, 20, 30, 40, 50, 60]` 这类更宽广的测试空间。
   - **多参数因子**（ MACD 的 `fast_n, slow_n, signal_n` 和 DualMA 的 `fast_n, slow_n`）：直接使用因子中的 `param_list` ，不需要扩展参数。
...
7. **输出结果**：每个因子不论测试多少组参数，最终只输出 1 个合并的回测结果文件：`{Category}_{FactorName}_BacktestResult.csv`
