...
# 代表意义：仅使用双均线因子，判断趋势大方向。结果保留原始偏离度信号。
# 适用板块：Ferrous
# ==============================================================================
...
def main():
    ...
    for FACTOR_MODE in ['RAW', 'STATE_MACHINE']:
        ...
        for sector, ts_codes in sector_map.items():
            ...
            for fast_ma, slow_ma in param_combinations:
                ...
                for ts_code in valid_symbols:
                    ...
                    if FACTOR_MODE == 'RAW':
                        # 信号原始值：快均线相对于慢均线的偏离度
                        states = (ma_fast - ma_slow) / ma_slow
                    else:
                        # 状态机映射 [-1, 0, 1]
        ...
        if results:
            ...
            report_path = os.path.join(output_dir, f"TrendMomentum_DualMACrossover_{FACTOR_MODE}_Ferrous_BacktestResult.csv")
            res_df.to_csv(report_path, encoding='utf-8-sig', index=False)
            print(f"[{FACTOR_MODE}] 回测结果报表已保存至: {report_path}")
            print(res_df)
        ...
        if best_positions:
            ...
            print(f"[{FACTOR_MODE}] 最优仓位文件已保存至: {pos_path}")
