"""
MovingAverage_V5_1  —  信号: fast_slow_gap_over_vol
    信号 = (fast_ma - slow_ma) / 收盘价滚动标准差
"""
import os
...
SIGNAL_DEF = 'fast_slow_gap_over_vol'

FAST_WINDOW_LIST  = [5, 10, 15, 20]
...
def calc_strength(df, fast_w, slow_w):
    close = df['adj_close']
    ...
    for slow_w in SLOW_WINDOW_LIST:
        for fast_w in FAST_WINDOW_LIST:
            ...
            for ts_code in ts_codes:
                strength_dict[ts_code] = calc_strength(data[ts_code], fast_w, slow_w)
            ...
            for zw in zw_values:
                ...
                for smooth_t in SMOOTH_T_LIST:
                    for mode in SIGNAL_MODE_LIST:
                        print(f'  {SIGNAL_DEF} F={fast_w} S={slow_w} ZW={zw if USE_ZSCORE else "noZ"} T={smooth_t} {mode}')
                        ...
                        name = (
                            f'MovingAverageV5_1_{sector}_{SIGNAL_DEF}_'
                            f'F_{fast_w}_S_{slow_w}_'
