"""
MovingAverage_V5_4  —  信号: slow_ma_slope_over_atr
    信号 = (slow_ma - slow_ma.shift(slp_k)) / ATR
"""
import os
...
SIGNAL_DEF = 'slow_ma_slope_over_atr'

SLOW_WINDOW_LIST    = [20, 40, 60, 80, 100, 120]
ATR_WINDOW_LIST     = [15]
SLOPE_LOOKBACK_LIST = [3]
Z_SCORE_WINDOW_LIST = [60]
USE_ZSCORE          = False   # False 则跳过 z-score，直接用原始信号
SMOOTH_T_LIST       = [1]
SIGNAL_MODE_LIST    = ['trend']

# fast_w 对 slope/atr 本身无影响，固定为 5 保持文件名一致
FAST_WINDOW = 5
...
def calc_strength(df, slow_w, atr_w, slp_k):
    close = df['adj_close']
...
for sector, ts_codes in sector_map.items():
    ...
    for atr_w in ATR_WINDOW_LIST:
        for slp_k in SLOPE_LOOKBACK_LIST:
            for slow_w in SLOW_WINDOW_LIST:

                strength_dict = {}
                ret_vol_dict  = {}
                for ts_code in ts_codes:
                    strength_dict[ts_code] = calc_strength(data[ts_code], slow_w, atr_w, slp_k)
                ...
                for zw in zw_values:
                    ...
                    for smooth_t in SMOOTH_T_LIST:
                        for mode in SIGNAL_MODE_LIST:
                            print(f'  {SIGNAL_DEF} S={slow_w} ATR={atr_w} SLP={slp_k} ZW={zw if USE_ZSCORE else "noZ"} T={smooth_t} {mode}')
                            ...
                            name = (
                                f'MovingAverageV5_4_{sector}_{SIGNAL_DEF}_'
                                f'F_{FAST_WINDOW}_S_{slow_w}_ATR_{atr_w}_SLP_{slp_k}_'
