...
| 均值回归 | 衰竭反转复合 | `ret=close.pct_change()`，`vol_z=(volume-mean(volume,N))/(std(volume,N)+1e-8)`，`intraday_range=(high-low)/(abs(open)+1e-8)`，`wick_bias=(upper_wick-lower_wick)/(high-low+1e-8)`，`-rolling_mean(ret*vol_z*(1+intraday_range)+0.5*wick_bias,N/2)` | 捕捉放量拉升/杀跌后的情绪衰竭与影线反转结构 ，比WickImbalance更有说服力|
...
| 均值回归 | 缺口回补压力 | `gap=open/(close.shift(1)+1e-8)-1`，`intraday_ret=close/(open+1e-8)-1`，`close_location=((close-low)-(high-close))/(high-low+1e-8)`，`rolling_mean(-sign(gap)*intraday_ret*(1+close_location),N)` | 衡量跳空后是否被盘中回补，结合收盘位置判断回补力度 |
