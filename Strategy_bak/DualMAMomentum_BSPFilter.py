# ==============================================================================
# 策略类型：趋势动量复合策略 + 盘口买卖压力(BuyingSellingPressure)微观因子过滤与增强
# 策略名称：DualMAMomentum_BSPFilter
# 代表意义：在基础的双均线+动量框架上，利用 BuyingSellingPressure 微观因子作为过滤/确认指标。
#          当大级别趋势与盘内多空力量产生共振时，放大持仓；产生背离时，削弱持仓防御震荡。
# 适用板块：Ferrous
# ==============================================================================
...
def main():
    ...
    # 买卖压力微观因子过滤参数
    USE_BSP_FILTER = True
    BSP_WINDOW = 20           # 微观因子的平滑窗口
    BSP_SOFT_WEIGHT = 0.5     # 出现背离时的削弱权重（例如大方向多头，但是收盘价持续受压在K线底部）
    BSP_ENHANCE_WEIGHT = 1.5  # 发生共振时的放大权重（例如向上突破且光头大阳线）
    WEIGHT_SMOOTH_DAYS = 10   # 权重乘数的平滑天数，防止跳跃
    ...
    for sector, ts_codes in sector_map.items():
        ...
        for fast_ma, slow_ma, mom_n in param_combinations:
            ...
            for ts_code in valid_symbols:
                ...
                if USE_BSP_FILTER:
                    # 每根K线的买盘压力占比：(收盘-最低)/(最高-最低)
                    pressure = (close - low) / (high - low + 1e-9)
                    pressure_ma = pressure.rolling(window=BSP_WINDOW).mean()
                    bsp_sig = (pressure_ma - 0.5) * 2
                    
                    # 趋势同向共振增强，背离防守减仓
                    weight.loc[(states > 0) & (bsp_sig > 0)] = BSP_ENHANCE_WEIGHT    # 处于多头，且买盘大于卖盘
                    weight.loc[(states < 0) & (bsp_sig < 0)] = BSP_ENHANCE_WEIGHT    # 处于空头，且卖盘大于买盘
                    weight.loc[(states > 0) & (bsp_sig <= 0)] = BSP_SOFT_WEIGHT      # 趋势多但是卖压大（比如长上影线连爆）
                    weight.loc[(states < 0) & (bsp_sig >= 0)] = BSP_SOFT_WEIGHT      # 趋势空但是买压大（比如逢低总有资金兜底）
    ...
    if results:
        ...
        report_path = os.path.join(output_dir, "DualMAMomentum_BSPFilter_BacktestResult.csv")
    ...
    if best_positions:
        ...
        pos_path = os.path.join(output_dir, "DualMAMomentum_BSPFilter_Position.csv")
