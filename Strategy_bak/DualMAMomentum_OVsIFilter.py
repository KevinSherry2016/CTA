# ==============================================================================
# 策略类型：趋势动量复合策略 + 隔夜日内差异(OVsI)微观因子过滤与增强
# 策略名称：DualMAMomentum_OVsIFilter
# 代表意义：在基础的双均线+动量框架上，利用 OvernightVsIntraday 微观因子作为过滤/确认指标。
#          当大级别趋势与微观的多空结构产生共振时，放大持仓；产生背离时，削弱持仓防御震荡。
# 适用板块：Ferrous
# ==============================================================================
...
def main():
    ...
    # OVsI微观因子过滤参数
    USE_OVI_FILTER = True
    OVI_WINDOW = 20           # 微观因子的平滑窗口
    OVI_SOFT_WEIGHT = 0.5     # 出现背离时的削弱权重（比如大多头趋势但日内被持续砸盘）
    OVI_ENHANCE_WEIGHT = 1.5  # 发生共振时的放大权重
        ...
        for fast_ma, slow_ma, mom_n in param_combinations:
            ...
            for ts_code in valid_symbols:
                ...
                if USE_OVI_FILTER:
                    ...
                    ovi_raw = (intraday_ret - overnight_ret).rolling(window=OVI_WINDOW).mean()
                    
                    # 趋势同向共振增强，背离防守减仓
                    weight.loc[(states > 0) & (ovi_raw > 0)] = OVI_ENHANCE_WEIGHT
                    weight.loc[(states < 0) & (ovi_raw < 0)] = OVI_ENHANCE_WEIGHT
                    weight.loc[(states > 0) & (ovi_raw <= 0)] = OVI_SOFT_WEIGHT
                    weight.loc[(states < 0) & (ovi_raw >= 0)] = OVI_SOFT_WEIGHT
    ...
    if results:
        ...
        report_path = os.path.join(output_dir, "DualMAMomentum_OVsIFilter_BacktestResult.csv")
    ...
    if best_positions:
        ...
        pos_path = os.path.join(output_dir, f"DualMAMomentum_OVsIFilter_Position.csv")
