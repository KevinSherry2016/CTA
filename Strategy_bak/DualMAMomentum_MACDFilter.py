# ==============================================================================
# 策略类型：趋势动量复合策略 + MACD 过滤与增强
# 策略名称：DualMAMomentum_MACDFilter
# 代表意义：在基础的双均线+动量框架长，引入MACD趋势同向增强过滤
# 适用板块：Ferrous
# ==============================================================================
...
def main():
        ...
        for fast_ma, slow_ma, mom_n in param_combinations:
            ...
            for ts_code in valid_symbols:
                ...
                if USE_MACD_FILTER:
                    ...
                    # 趋势同向增强，反向减弱
                    weight.loc[(states > 0) & (macd_diff > 0)] = MACD_ENHANCE_WEIGHT
                    weight.loc[(states < 0) & (macd_diff < 0)] = MACD_ENHANCE_WEIGHT
                    weight.loc[(states > 0) & (macd_diff <= 0)] = MACD_SOFT_WEIGHT
                    weight.loc[(states < 0) & (macd_diff >= 0)] = MACD_SOFT_WEIGHT
    ...
    if results:
        ...
        report_path = os.path.join(output_dir, "DualMAMomentum_MACDFilter_BacktestResult.csv")
    ...
    if best_positions:
        ...
        pos_path = os.path.join(output_dir, f"DualMAMomentum_MACDFilter_Position.csv")
