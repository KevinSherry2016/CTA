...
# 策略类型：趋势策略
# 策略名称：DualMAThresh
# 代表意义：仅使用双均线因子，增加阈值过滤，只有快线偏离慢线超过阈值才开仓。结果映射到状态机[-1, 0, 1]
# 适用板块：Ferrous
# ==============================================================================
...
def main():
    ...
    THRES_LIST = [0.001, 0.003, 0.005, 0.01]  # 阈值列表
    ...
    for sector, ts_codes in sector_map.items():
        ...
        param_combinations = list(itertools.product(FAST_MA_LIST, SLOW_MA_LIST, THRES_LIST))
        
        for fast_ma, slow_ma, thres in param_combinations:
            if fast_ma >= slow_ma:
                continue
            ...
            for ts_code in valid_symbols:
                ...
                # 状态机映射 [-1, 0, 1]
                # 增加阈值逻辑：快线超过慢线(1+thres)做多，低于慢线(1-thres)做空，中间地带为空仓 (0.0)
                states = pd.Series(0.0, index=close.index)
                states[ma_fast > ma_slow * (1 + thres)] = 1.0
                states[ma_fast < ma_slow * (1 - thres)] = -1.0
            ...
            if pnl_sm:
                ...
                if sharpe > max_metric:
                    max_metric = sharpe
                    best_params = (fast_ma, slow_ma, thres)
    ...
    if results:
        ...
        report_path = os.path.join(output_dir, "DualMAThresh_Ferrous_BacktestResult.csv")
    ...
    if best_positions:
        ...
        pos_path = os.path.join(output_dir, f"DualMAThresh_Position_Ferrous.csv")
