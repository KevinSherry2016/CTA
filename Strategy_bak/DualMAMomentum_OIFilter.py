# ==============================================================================
# 策略类型：趋势动量复合策略 + OI持仓量 过滤与增强
# 策略名称：DualMAMomentum_OIFilter
# 代表意义：在基础的双均线+动量框架长，引入持仓量（OI）变化来确认趋势
# 适用板块：Ferrous
# ==============================================================================
...
def main():
    ...
    for sector, ts_codes in sector_map.items():
        ...
        for fast_ma, slow_ma, mom_n in param_combinations:
            ...
            for ts_code in valid_symbols:
                ...
                if USE_OI_FILTER and 'oi' in df.columns:
                    oi_short_ma = df['oi'].rolling(OI_SHORT).mean()
                    oi_long_ma = df['oi'].rolling(OI_LONG).mean()
                    
                    # 增仓趋势确认，减仓趋势存疑
                    weight.loc[oi_short_ma > oi_long_ma] = OI_ENHANCE_WEIGHT
                    weight.loc[oi_short_ma < oi_long_ma] = OI_SOFT_WEIGHT
    ...
    if results:
        ...
        res_df.to_csv(os.path.join(output_dir, "DualMAMomentum_OIFilter_BacktestResult.csv"), encoding='utf-8-sig', index=False)

    if best_positions:
        df_pos_sm = pd.DataFrame(best_positions, index=tradingDayList).fillna(0)
        df_pos_sm.to_csv(os.path.join(output_dir, f"DualMAMomentum_OIFilter_Position.csv"), encoding='utf-8-sig')
