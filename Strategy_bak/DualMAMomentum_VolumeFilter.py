# ==============================================================================
# 策略类型：趋势动量复合策略 + 成交量（Volume） 过滤与增强
# 策略名称：DualMAMomentum_VolumeFilter
# 代表意义：在基础的双均线+动量框架长，引入成交量（Volume）变化来确认价格趋势
# 适用板块：Ferrous
# ==============================================================================
...
def main():
    ...
    for sector, ts_codes in sector_map.items():
        ...
        for fast_ma, slow_ma, mom_n in param_combinations:
            ...
            for ts_code in valid_symbols:
                ...
                vol_sd = daily_ret.rolling(FINAL_VOL_WINDOW, min_periods=1).std().replace(0, np.nan)
                ...
                if USE_VOL_FILTER and 'vol' in df.columns:
                    v_short_ma = df['vol'].rolling(VOL_SHORT).mean()
                    v_long_ma = df['vol'].rolling(VOL_LONG).mean()
                    
                    weight.loc[v_short_ma > v_long_ma] = VOL_ENHANCE_WEIGHT
                    weight.loc[v_short_ma < v_long_ma] = VOL_SOFT_WEIGHT

                if WEIGHT_SMOOTH_DAYS > 1:
                    weight = weight.ewm(span=WEIGHT_SMOOTH_DAYS, adjust=False).mean()
                
                pos_sm = (states / vol_sd) * weight
    ...
    if results:
        pd.DataFrame(results).to_csv(os.path.join(output_dir, "DualMAMomentum_VolumeFilter_BacktestResult.csv"), encoding='utf-8-sig', index=False)
    if best_positions:
        pd.DataFrame(best_positions, index=tradingDayList).fillna(0).to_csv(os.path.join(output_dir, f"DualMAMomentum_VolumeFilter_Position.csv"), encoding='utf-8-sig')
