# ==============================================================================
# 策略类型：趋势动量持仓量复合策略
# 策略名称：DualMAMomentumOI
# 代表意义：以双均线因子为趋势主逻辑，动量因子为小趋势辅助，持仓量（Open Interest）作为趋势确认。
# 如果持仓量放大说明趋势动能增强，适当放大仓位；缩量则缩小仓位。同时规避换月期的跳空影响。
# 适用板块：Ferrous
# ==============================================================================
...
def main():
    ...
    MOM_N_LIST = [3, 5, 10]
    OI_MA_LIST = [5, 10, 20] # 新增：持仓量移动平均天数
    ...
    for sector, ts_codes in sector_map.items():
        ...
        param_combinations = list(itertools.product(FAST_MA_LIST, SLOW_MA_LIST, MOM_N_LIST, OI_MA_LIST))
        
        for fast_ma, slow_ma, mom_n, oi_n in param_combinations:
            ...
            for ts_code in valid_symbols:
                ...
                # 3. 趋势确认：持仓量变化
                oi_ma = oi.rolling(oi_n).mean()
                t_oi = pd.Series(False, index=close.index)
                t_oi[oi > oi_ma] = True # 持仓量大于均值，视为增仓/维持高持仓
                
                # 换月掩码：如果在持仓量计算窗口（oi_n）内发生过换月，屏蔽持仓量信号跳动
                if 'mapping_ts_code' in df.columns:
                    is_roll = df['mapping_ts_code'] != df['mapping_ts_code'].shift(1)
                    is_roll.iloc[0] = False # 首日不算换月
                    is_roll_period = is_roll.rolling(oi_n, min_periods=1).max() > 0
                else:
                ...
                # 2. 根据持仓量调整：持仓量较高适当增加（*1.2），持仓量低适当减少（*0.8）
                states[t_oi] = states[t_oi] * 1.2
                states[~t_oi] = states[~t_oi] * 0.8
            ...
            if pnl_sm:
                ...
                if sharpe > max_metric:
                    max_metric = sharpe
                    best_params = (fast_ma, slow_ma, mom_n, oi_n)
    ...
    if results:
        ...
        report_path = os.path.join(output_dir, "DualMAMomentumOI_Ferrous_BacktestResult.csv")
    ...
    if best_positions:
        ...
        pos_path = os.path.join(output_dir, f"DualMAMomentumOI_Position_Ferrous.csv")
